	...
	return (
		<section>
			...
			<p>
				...
				Since the aim was to further my understanding of neural networks, I
				opted not to use a framework like PyTorch, and instead implemented the
				neural networks using numpy matrices.
			</p>
			<section>
				<h3>The Problem</h3>
				<p>
					...
					letters. In order to circumvent this, I decided to procede using a basic form of
			...
			<section>
				...
				<p>
					As there are various styles of written Manchu, a lexigraph is first sorted into one of
					four <span className="bold">classes</span>. I defined the classes by comparing three
					features in various Manchu fonts: line weight, whether the letters romanized as "m" and
					"l" are connected to or disconnected from the center line, and whether the letters
					romanized as "a" and "e" are pointy or rounded. The actual classification is performed
					by an RNN, which I call the <span className="bold">Classifier</span>, which reads the
					image line by line.
				</p>
				<p>
					OCR is performed by two neural networks, each with four instances, one for each lexigraph
					class. For example, a lexigraph of class A is analyzed by a neural network trained only on
					class A lexigraphs, likewise for classes B, C, and D. The first networks, henceforth called
					...
					thinks the middle row of pixels corresponds to. The Primary OCR has two hidden layers.
				</p>
				<p>
				...
				<p>
					Finally, the output of the Secondary OCR is parsed and rendered as Unicode text. This is
					done on the frontend. The server sends the frontend the Secondary OCR's output as an array
					of characters, which may be any character in the Manchu Unicode block (a subset of the
					Mongolian block), a " " character, or a "*". A " " represents a pixel thought to be
					whitespace, and a "*" represents a break between two characters. To minimize the effect
					of noisy data, any characters which have no identical neighbors are deleted. For example,
					in the array ['a', 'a', 'b', 'a'], 'b' would be deleted because neither of its neighbors
					match it. Once this is done, all identical neighboring characters are collapsed into one.
					For example, ['a', 'a', 'b', 'b', 'b', 'c'] becomes ['a', 'b', 'c']. At this point, any
					'*' characters are removed. Their removal after the collapsing of neighboring characters
					allows the program to account for two letters in sequence. For example, ['a', 'a', '*', 'a']
					becomes ['a', 'a']. At this point the array is joined to create a string as the final
					output.
				</p>
			</section>
			<section>
				...
				<p>
					...
					Using whole words rather than manually labelled characters would make it much easier
					to assemble a large body of training data. As of the time of writing, there are 166
					lexigraphs in the training data, which is quite a small training set.
				</p>
				<p>
					Another change I would like to make in a future version is to add positional encoding.
					Right now, the Primary OCR's only hint to what part of the word it's looking at is
					the slice of the image it's given. It may be helpful to have another input which tells
					it whether that slice is at the beginning, end, or middle of a word.
				</p>
			</section>
		</section>
	);
