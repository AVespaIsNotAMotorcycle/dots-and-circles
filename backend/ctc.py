...
class CTCNeuralNetwork:
    LEARNING_RATE = 0.001
    _use_file = True
    NN_FILE_PATH = "saved_ann.json"
    INPUT_LAYER_SIZE = constants.OUTPUT_LAYER_SIZE * 21

    def __init__(self, hidden_layer_size=64):
        self.weight_x_h = randn(hidden_layer_size, self.INPUT_LAYER_SIZE) / 1000
    ...
    def digits_array_to_x(self, array):
        x = np.zeros((self.INPUT_LAYER_SIZE, 1))
        for index, value in enumerate(array):
            x_index = (index * constants.OUTPUT_LAYER_SIZE) + value
            x[x_index] = 1
        return x
    ...
    def predict_tokens(self, inputs, labels=[], backprop=False):
        ...
        for index in range(len(inputs)):
            input = inputs[index]
            if backprop: label = labels[index]
    ...
    def train_on_tokens(self, inputs, labels):
        return self.predict_tokens(inputs, labels, True)
